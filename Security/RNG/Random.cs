...
#region Public Method
    ...
    /// <inheritdoc/>
    public virtual string AlgorithmName() { return "Random"; }
    ...
    /// <inheritdoc/>
    public virtual byte NextByte() { return this.NextBytes(1)[0]; }
    ...
    public virtual T Choice<T>(T[] items) {
      if (items.Length > int.MaxValue) {
        throw new ArgumentOutOfRangeException(
            nameof(items),
            $"The items length or size can't be greater than int.MaxValue or { int.MaxValue }.");
      }
    ...
    /// <inheritdoc/>
    public override string ToString() { return this.AlgorithmName(); }
